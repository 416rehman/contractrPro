...
export const loadExpenses = async (currentOrganizationId: string) => {
  const { lastRequestedOn } = useExpensesStore.getState();
  if (!lastRequestedOn) {
  ...
  try {
    ...
    if (orgExpenses.length !== currentExpenses.length || orgExpenses.some((expense: any, i: number) => expense.id !== currentExpenses[i].id)) {
...
export const updateExpenses = async (expense: Expense, currentOrganizationId: string) => {
  ...
  try {
    if (currentOrganizationId) {
      ...
      expense.ExpenseEntries = expense?.ExpenseEntries.filter((entry: any) => entry.name || entry.unitCost || entry.quantity || entry.description) || [];
