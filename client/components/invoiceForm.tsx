...
export default function InvoiceForm({ id, className }: Props) {
  ...
  const onChangeHandler = (e: React.ChangeEvent<HTMLInputElement>) => {
    setEditedInvoice({ ...editedInvoice!, [e.target.name]: e.target.value } as Invoice);
  };
  ...
  const onEntryChangedHandler = (name: string, value: any, id: string) => {
    ...
    if (!editedInvoice?.InvoiceEntries?.find((entry: any) => entry.id === id)) {
      setEditedInvoice((prev: Invoice | null | undefined) => {
        if (!prev) return prev;
        return {
          ...prev,
    ...
    // otherwise, update the entry with the given id
      ...
      setEditedInvoice((prev: Invoice | null | undefined) => {
        ...
        return {
          ...prev,
          InvoiceEntries: prev?.InvoiceEntries?.map((entry: any) => entry.id === id ? { ...entry, [name]: value } : entry)
        } as Invoice
      });
    }
  };
  ...
  const onEntryDeleteHandler = (id: string) => {
    ...
    setEditedInvoice((prev: Invoice | null | undefined) => {
      ...
      return {
        ...prev,
        InvoiceEntries: prev?.InvoiceEntries?.filter((entry: any) => entry.id !== id)
      } as Invoice
    });
  };
  ...
  const exportAsCSV = () => {
    ...
    const values = editedInvoice.InvoiceEntries.map((job: any) => Object.values(job).join(",")).join("\n");
              ...
              <ClientSelector
                ...
                onClientChange={(changedClients) => {
                  if (changedClients.length > 0 && changedClients[0]?.id) {
                    setEditedInvoice((prev: Invoice | null | undefined) => {
                      if (!prev) return prev;
                      return { ...prev, BillToClientId: changedClients[0]?.id } as Invoice
                    });
                  }
                }}
                selectedClientIds={editedInvoice?.BillToClientId ? [editedInvoice.BillToClientId] : []}
              />
              ...
              <div className={"flex flex-row gap-4 justify-between"}>
                ...
                <div className={"flex flex-col gap-2 items-end"}>
                  <span className={"text-sm font-medium text-default-500"}>Subtotal: $<span
                    className={"text-default-800"}>{editedInvoice?.InvoiceEntries?.reduce((acc: number, entry: any) => acc + entry.unitCost * entry.quantity, 0)}</span></span>
                  <span className={"text-sm font-medium text-default-500"}>Tax: $<span
                    className={"text-default-800"}>{(editedInvoice?.InvoiceEntries?.reduce((acc: number, entry: any) => acc + entry.unitCost * entry.quantity, 0) * (editedInvoice?.taxRate || 0) / 100).toFixed(2)}</span></span>
                  <span className={"text-medium font-medium text-default-500"}>Total: $<span
                    className={"text-default-800"}>{(editedInvoice?.InvoiceEntries?.reduce((acc: number, entry: any) => acc + entry.unitCost * entry.quantity, 0) * (1 + (editedInvoice?.taxRate || 0) / 100)).toFixed(2)}</span></span>
            ...
            <div className={"flex flex-col gap-1 items-start"}>
              {invoice?.updatedAt &&
                <Tooltip content={invoice?.updatedAt}>
                  <span className={"text-xs text-default-500"}>Updated {formatDistanceToNow(new Date(invoice?.updatedAt), { addSuffix: true })}</span>
                </Tooltip>
              }
              {invoice?.createdAt && (
                <Tooltip content={invoice?.createdAt}>
                  <span className={"text-xs text-default-500"}>Created {formatDistanceToNow(new Date(invoice?.createdAt), { addSuffix: true })}</span>
                </Tooltip>
              )}
            </div>
