...
export default function VendorsSidebar({ className }: Props) {
  ...
  const handleFilterChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    vendors.filter((item: any) => item.name.toLowerCase().includes(e.target.value.toLowerCase()));
  };
  ...
  const sidebar = <Card shadow={"none"} isBlurred={true}
    ...
    <CardFooter>
      <Button variant={"ghost"} className={"flex-grow"} href={"/vendors/new"} as={NextLink}>
        Create New Vendor
      </Button>
    </CardFooter>
  </Card>;
