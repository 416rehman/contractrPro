...
module.exports.createErrorResponse = (message, err) => {
    ...
    try {
        ...
        if (err?.code) {
            switch (err?.code) {
                case 'P2000': {
                    hint = `The provided value for the column is too long for the column's type: ${
                        target || 'This value'
                    }`
                    break
                }
                case 'P2001': {
                    hint = `The record you're looking for doesn't exist: ${
                        target || 'Record'
                    }`
                    break
                }
                ...
                case 'P2003': {
                    hint = `The foreign key constraint failed on the field: ${
                        target || 'Field'
                    }`
                    break
                }
                case 'P2004': {
                    hint = `A constraint failed on the database: ${
                        target || 'Database'
                    }`
                    break
                }
                ...
                case 'P2006': {
                    hint = `The provided ${target || 'value'} for ${
                        err?.meta?.model
                    } field ${err?.meta?.field} is not valid`
                    break
                }
                case 'P2007': {
                ...
                case 'P2011': {
                    hint = `Null constraint violation on the ${
                        target || 'constraint'
                    }`
                    break
                }
    ...
    // if error is ZodError set message to "Z Validation Error"
    if (err?.issues) {
