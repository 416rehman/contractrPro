...
describe(`POST /organizations/:org_id/contracts`, () => {
    ...
    it(`Should return success for creating a contract for an existing organization`, async () => {
        ...
        const response = await request(app).post(`/organizations/${orgID}/contracts`).send(contractData).expect(201)

        expect(response.body.status).toBe('success')
        expect(response.body).toHaveProperty('data')
        expect(response.body.data).toHaveProperty('id')
        expect(response.body.data).toHaveProperty('name', contractData.name)
        expect(response.body.data).toHaveProperty('description', contractData.description)
        expect(response.body.data).toHaveProperty('startDate', contractData.startDate.toISOString())
        expect(response.body.data).toHaveProperty('dueDate', contractData.dueDate.toISOString())
        expect(response.body.data).toHaveProperty('completionDate', contractData.completionDate.toISOString())
    ...
    it(`Should return error for creating a contract without requiring the organization's ID`, async () => {
        ...
        const res = await request(app).post(`/organizations/${orgID}/contracts`).send(contractData)
    ...
    it(`Should return error for creating a contract for an organization that doesn't exist`, async () => {
        ...
        const response = await request(app).post(`/organizations/${orgID}/contracts`).send(contractData)
        .expect((res) => res.status === 400 || res.status === 422) // Express-validator returns 422 for validation errors
