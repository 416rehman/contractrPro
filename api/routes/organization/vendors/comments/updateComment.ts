...
export default async (req, res) => {
    try {
        ...
        await db.transaction(async (tx) => {
            ...
            const [comment] = await tx.update(comments).set(body).where(and(eq(comments.id, commentId), eq(comments.organizationId, orgId), eq(comments.vendorId, vendorId))).returning();
