 ...
 *   post:
 *     summary: Add a member to a contract
 *     tags: [ContractMembers]
 *     responses:
 *       200:
 *         description: Contract member added
 */
export default async (req, res) => {
    try {
        ...
        const body = { ...pick(req.body, ['permissions']) }
        ...
        if (existing) {
            await db.update(contractMembers).set(body).where(eq(contractMembers.id, existing.id));
