    ...
    try {
        ...
        await db_1.db.transaction(async (tx) => {
            ...
            if ((!comment.content || comment.content === '') &&
                ...
                const deletedComments = await tx.delete(db_1.comments).where((0, drizzle_orm_1.eq)(db_1.comments.id, commentId)).returning();
                ...
                if (targetAttachment) {
                    const deleted = await tx.delete(db_1.attachments).where((0, drizzle_orm_1.eq)(db_1.attachments.id, attachmentId)).returning();
